      SUBROUTINE SOLAR(TF,LAST,FNC,NST)
	  
C-kb This file contains edits made by Katy Browning (UCL) in August 2023
C-kb to extend the existing Clima calculation and output of solar fluxes
C-kb for the total solar spectrum to any one or more user-specified
...
C-kb All code edits are flagged with the "C-kb" or "!kb" markers and are
C-kb Fortran77/90 compliant.
...
!gna - changed ngs to 8 to account for c2h6
...
C-kb Line added
     ...
     &   KAPPALAYERSOL_H2O   !EWS - removed BETA because it wasn't used 9/4/2015
...
!     & ,kmatrix_sol(NSOL,IK) ! c-rr Added weightco2_h2O data array and weights array that combines co2_h2o and Ch4 weight arrays 3/19/2012! c-rr Added 16 weight coefficient array for CO2-H2O kspectrum mixed absorption coefficiets, 3/15/2012
     ...
     & CPN(ND), CPNT(ND)
      ...
      DIMENSION  TAUAS(ND-1),TAUS(ND-1), ! EWS - removed TAUA(ND-1) because it wasn't used 9/4/2015
     ...
     &  SOLAV(NSOL+1) ! Added solar intervals
      DIMENSION TAUCONTSOL(ND),  ! Added water continuum tau for solar 8/30/2012 ! Removed W(NF), ALAM(NF), AVOLD(NF) since they weren't used 9/4/2015 - EWS
...
C
 ...
 1152 CONTINUE
C-kb Lines added
C-kb Note, the I counter used below corresponds to solar spectrum bins.
C-kb It's not the same as the I counter above that counts the atmosphere
C-kb layer boundaries and should more appropriately have been labelled
C-kb as J to be consistent with its use elsewhere in the code.  
      IF (NSPEC .NE. 0) THEN
         DO 810 I = 1, NSOL
            FDNSOLI(I,1) = 0.0
            FUPSOLI(I,1) = 0.0
...
c 3456     format(1p4e14.5,3(2x,i3)) !EWS - not used
...
!       DO 1180 K3 = 1, NPR3   !**BEGIN NPR3 LOOP** K3 is gas 2 (CH4) from nearIR_expsums.pdat
                DO 1175 K2 = 1, IK  ! K2 is kspectrum CO2 coefficients from solar38_h2o.dat
                   ...
                   AP = weights(1, I, K1)*weights(2,I,K2)   ! Only H2O & CO2

!                   AP = weights(1, I, K1)*weights(2,I,K2)*
!     &             WGHTO2(I,KO2)  ! Only convolve with O2
!                   AP = weights(1, I, K1)*weights(2,I,K2)*
!     &             weights(3,I,K3)*WGHTO2(I,KO2)  ! Only convolve with O2
! coefficients in interval 16 c-rr 5/20/2012.
...
c           print 9999,AFT,pp
c 9998        format(1PE12.5,2x,I2) !EWS - not used
      END IF
...
C-kb Lines added
             IF (NSPEC .NE. 0) THEN
               IF (J .EQ. 1) THEN		 
                   FDNSOLI(I,1) = FDNSOLI(I,1) + SOLINT(I)*FDNA(J)
                   FUPSOLI(I,1) = FUPSOLI(I,1) + SOLINT(I)*FUPA(J)
               ELSE IF (J .EQ. ND) THEN
                   FDNSOLI(I,2) = FDNSOLI(I,2) + SOLINT(I)*FDNA(J)
                   FUPSOLI(I,2) = FUPSOLI(I,2) + SOLINT(I)*FUPA(J)
               END IF
             END IF  
C-kb End of lines added
...
c 99999 format(1x,'i =',i2,2x,1pe10.3) !EWS - not used
