...
C-kb *** The edits made in this program file ... ***
...
C-kb Line added
      DIMENSION FDNSOLIT(NSOL,NTOPBOT), FUPSOLIT(NSOL,NTOPBOT)
...
C-kb Lines added
      IF (NSPEC .NE. 0) THEN
         DO 810 I = 1, NSOL
            FSOLI(I) = SOLCON * SOLINT(I)
  810    CONTINUE
         CALL CONVSOLG     !kb - convert group wavelengths to pointers on solar spectrum
      END IF
...
C-kb Lines added
          IF (NSPEC .NE. 0) THEN
             DO 830 I = 1, NSOL
                FDNSOLIT(I,1)=FDNSOLIT(I,1)+FDNSOLI(I,1)*WEIGHTT
                FUPSOLIT(I,1)=FUPSOLIT(I,1)+FUPSOLI(I,1)*WEIGHTT
                FDNSOLIT(I,2)=FDNSOLIT(I,2)+FDNSOLI(I,2)*WEIGHTT
                FUPSOLIT(I,2)=FUPSOLIT(I,2)+FUPSOLI(I,2)*WEIGHTT
  830        CONTINUE
          END IF   
C-kb End of lines added
...
C-kb Lines added
      IF (NSPEC .NE. 0) THEN
         DO 840 I = 1, NSOL
            FDNSOLI(I,1) = SOLCON * 0.5 * FDNSOLIT(I,1)
            FUPSOLI(I,1) = SOLCON * 0.5 * FUPSOLIT(I,1)
            FDNSOLI(I,2) = SOLCON * 0.5 * FDNSOLIT(I,2)
            FUPSOLI(I,2) = SOLCON * 0.5 * FUPSOLIT(I,2)
  840    CONTINUE
      END IF
C-kb End of lines added
